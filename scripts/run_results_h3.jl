...
cheb_funcs = cheb_functions(n, repeat(-maxes,k), repeat(maxes,k))
d_cheb_funcs = d_cheb_functions(n, repeat(-maxes,k), repeat(maxes,k))

cheb_basis = FunctionBasis(
    cheb_funcs,
    d_cheb_funcs
)

cheb_indices = matrix_poly_multiindices(k*d,0,n)

ch = cheb_h(cheb_basis, cheb_indices)
cgh = cheb_grad_h(cheb_basis, cheb_indices)
